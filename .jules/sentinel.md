...
## 2025-05-11 - Prevent ReDoS and App DoS via Strict Input Checks and Bounded Regex
**Vulnerability:** The API chat endpoint did not check the size or type of incoming `message` fields, causing a potential Application DoS via oversized requests. Furthermore, regex parsing (`extractImageQuery`) utilized unbounded quantifiers (e.g., `.+?`) without preliminary length limits, creating a risk of Regular Expression Denial of Service (ReDoS).
**Learning:** Checking for truthiness (`!message`) is insufficient for string-based operations or regex execution. Regexes operating on user input should be bounded (using `{1,100}` rather than `+` or `*`), and inputs should have explicit length cutoffs to prevent algorithmic complexity attacks.
**Prevention:** Always strictly enforce parameter types (`typeof message === 'string'`) and bounded lengths on raw API payloads, and never process untrusted input with unbounded regex quantifiers.
