...
## 2025-05-24 - Prevent ReDoS and DoS via Payload Length Limits
**Vulnerability:** The `/api/chat` endpoint and `extractImageQuery` function lacked maximum length validations for user-provided strings. This allowed attackers to send massive payloads, leading to application-level Denial of Service (DoS) due to excessive memory consumption or Regular Expression Denial of Service (ReDoS) when large strings were parsed by complex regex patterns.
**Learning:** Always implement strict, reasonable upper bounds on the length of user-provided strings, especially before they are processed by regular expressions or core application logic.
**Prevention:** Add explicit length checks (e.g., `message.length > 50000` or `message.length > 500`) alongside type validation (`typeof message !== 'string'`) early in the request lifecycle to fail securely and prevent resource exhaustion.
