...
## 2025-05-12 - Prevent DoS via Missing Input Validation on Arrays and Strings
**Vulnerability:** The `/api/chat` POST endpoint accepted potentially unbounded string sizes for `message` and `systemInstruction`, and did not type check arrays (`history`, `attachments`) leading to potential memory exhaustion or Application DoS when iterating or processing downstream.
**Learning:** Always explicitly bound string sizes (e.g., `< 50000`) and strictly enforce type constraints (e.g., `typeof === 'string'`, `Array.isArray`) early in the request lifecycle for public-facing endpoints.
**Prevention:** Add hard limits to incoming payload string properties and robust type assertions before initiating further logic or API calls.
