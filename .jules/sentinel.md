...
## 2025-02-27 - Fix rate limiting behind reverse proxy
**Vulnerability:** The `express-rate-limit` middleware was ineffective because the application is deployed behind a reverse proxy (Vercel/Cloudflare). Without `trust proxy` configured in Express, `req.ip` represents the proxy's IP rather than the actual client IP, causing global rate limiting (a Denial of Service risk for all users) instead of per-user blocking.
**Learning:** IP-based security middleware relies entirely on correct IP resolution. When hosted on PaaS providers, Express defaults to `req.socket.remoteAddress` which will always be the internal load balancer.
**Prevention:** Always ensure `app.set('trust proxy', 1)` (or equivalent depending on proxy layers) is configured immediately after initializing an Express app when deploying behind services like Vercel, Heroku, or Cloudflare, especially if relying on IP-based tracking.
