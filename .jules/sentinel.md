...
## 2024-05-01 - Express Route Missing Express Trust Proxy + Missing Type Validation
**Vulnerability:** The Express rate limiter was incorrectly limiting based on the reverse proxy's IP address (due to missing `app.set('trust proxy', 1)`), potentially locking out all users or allowing rate-limit bypass. Additionally, unvalidated request body parameters were directly passed to `Buffer.from(audioBase64, 'base64')`, exposing the app to potential crashes and Application DoS if an object was passed instead of a string.
**Learning:** Middleware relying on client IPs (like express-rate-limit) requires explicit trust proxy configuration when deployed behind services like Vercel or Cloudflare. Unvalidated JSON inputs reaching native methods like `Buffer.from` can throw unhandled exceptions, crashing the application and leaking internal errors.
**Prevention:** Always configure `trust proxy` in Express when using IP-based rate limiting behind a reverse proxy. Explicitly validate the types of request payload properties (e.g., `typeof input === 'string'`) before using them in native functions. Ensure error catch blocks return generic messages.
