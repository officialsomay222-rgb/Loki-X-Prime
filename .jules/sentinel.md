...
\n## 2025-05-02 - [Rate Limiting Denial of Service Behind Proxy]\n**Vulnerability:** The Express rate limiter was blocking legitimate users because it was configured behind a reverse proxy (Vercel/Cloudflare) without trusting the proxy's IP. All requests appeared to come from the same proxy IP, triggering the global rate limit rapidly.\n**Learning:** When using `express-rate-limit` in a hosted environment behind a proxy, Express defaults to reading the socket's remote IP (the proxy) instead of the `X-Forwarded-For` header.\n**Prevention:** Always explicitly add `app.set('trust proxy', 1)` in Express applications that rely on client IP addresses (like rate limiters) when deployed behind reverse proxies.
