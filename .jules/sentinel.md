...
## 2025-05-18 - Prevent ReDoS and Payload DoS via Input Constraints
**Vulnerability:** Unbounded strings in Express endpoints (e.g., `message` in `/api/chat`) and loose regex matching (e.g., `extractImageQuery`) allowed extremely large payloads or malicious inputs to cause Event Loop blocking (ReDoS) or memory exhaustion before handing off to AI SDKs.
**Learning:** Checking truthiness (`!message`) or type (`typeof message !== 'string'`) is insufficient when dealing with complex regexes or downstream external dependencies. Bounded quantifiers (`.{1,100}`) inside regex groups often cause truncation regressions; preventing ReDoS is better handled via an early strict length cutoff on the input itself.
**Prevention:** Always enforce a strict character limit (`message.length > 500` for regex functions, or `50000` for LLM contexts) and explicit type constraints (`Array.isArray(history)`) immediately after extracting the body in Express routes.
