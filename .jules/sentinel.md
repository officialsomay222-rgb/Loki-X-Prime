...
## 2025-04-25 - Prevent Information Leakage and Enforce Input Validation
**Vulnerability:** The Express backend (`api/index.ts`) leaked raw `error.message` strings directly to the client on 500 errors (Information Exposure), and did not enforce string type validation on `req.body.audioBase64` and `req.body.text` inputs, which could lead to `TypeError` exceptions if objects/arrays were passed.
**Learning:** Returning unhandled exception messages (like `ENOENT` paths) can expose sensitive internal server structure (like temp directory paths). Not validating the input type before passing it to native functions like `Buffer.from` can lead to Application DoS via crashes.
**Prevention:** Always explicitly validate that `req.body` inputs match expected types (e.g., `typeof audioBase64 === 'string'`) and return generic error messages (e.g., `"Internal server error"`) to the client on unexpected failures, while using `console.error` to log the exact `error.message` for internal debugging.
