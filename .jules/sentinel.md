...
## 2025-05-20 - Prevent Type Manipulation and DoS via Payload Validation
**Vulnerability:** The `/api/chat` endpoint previously failed to explicitly check the types and lengths of incoming `message` and `history` arguments. Allowing excessively long strings, or non-string/non-array types to be processed can trigger ReDoS in downstream matchers or memory exhaustion/crashes when passed to native Node or SDK functions expecting specific structures.
**Learning:** Checking for truthiness (`if (!message)`) is insufficient validation for complex APIs. An attacker could bypass truthy checks with deeply nested arrays or multi-megabyte strings, bypassing intended limits and potentially crashing the process.
**Prevention:** Always enforce explicit type bounds (e.g. `typeof message !== 'string'`) and strict maximum length constraints (e.g. `message.length > 50000`) at the very beginning of the controller. Fail securely by returning a generic 400 error immediately.
