...
## 2025-05-06 - Prevent DoS via Array Injection in Chat API
**Vulnerability:** The `/api/chat` endpoint parsed `req.body` directly using `express.json()` and extracted variables like `message`, `history`, and `attachments` without verifying their types. Passing non-string types (like arrays or objects) to `message.trim()` or string-only functions would throw a `TypeError` and crash the endpoint, leading to a potential DoS condition for that route.
**Learning:** Destructuring JSON body parameters in an Express API does not guarantee type safety. Missing explicit type checks allows malicious payloads to bypass validation and trigger unexpected exceptions in native functions.
**Prevention:** Always implement explicit type validation for incoming JSON bodies in Express controllers (e.g., `typeof message !== 'string'`, `!Array.isArray(history)`) before passing data to underlying logic or external SDKs.
