...
## 2026-05-10 - Prevent ReDoS and Payload DoS in API
**Vulnerability:** The `extractImageQuery` function used unbounded quantifiers (`.+`) in its regex matchers, exposing the system to Regular Expression Denial of Service (ReDoS). Additionally, the `/api/chat` route lacked payload size validation for the `message` parameter and type checking for the `history` parameter, opening vectors for application-level DoS attacks and unhandled exceptions.
**Learning:** Naively consuming arbitrary lengths of user-submitted text into regular expressions or application payloads enables trivial resource exhaustion attacks. Unbounded quantifiers combined with malicious inputs can cause catastrophic backtracking.
**Prevention:** Always convert unbounded regex quantifiers (`.+` or `.*`) to bounded equivalents (e.g., `.{1,100}`) when evaluating user input. At the route level, enforce strict payload constraints (e.g., limiting chat messages to reasonable lengths, like 50,000 characters) and explicitly validate parameter types (e.g., `typeof message === 'string'` and `Array.isArray(history)`).
