...
export class ErrorBoundary extends Component<Props, State> {
  ...
  public render() {
    if (this.state.hasError) {
      return (
        <div className="flex flex-col items-center justify-center flex-1 w-full min-h-[400px] bg-slate-950 text-white p-6 text-center">
          ...
          <div className="max-w-md mx-auto mb-8 p-4 bg-red-500/10 border border-red-500/20 rounded-xl text-xs text-red-400 font-mono text-left overflow-auto max-h-40 custom-scrollbar">
            {(() => {
              try {
                if (this.state.error && typeof this.state.error.message === 'string' && this.state.error.message.startsWith('{')) {
                  return <pre>{JSON.stringify(JSON.parse(this.state.error.message), null, 2)}</pre>;
                }
              } catch (e) {
                // Fallback if JSON.parse fails
              }
              return <p>{this.state.error ? String(this.state.error.message || this.state.error) : 'Unknown error'}</p>;
            })()}
          </div>
          <button 
